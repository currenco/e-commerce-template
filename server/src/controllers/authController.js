...
export const logout = catchAsyncErrors(async (req, res, _next) => {
  res
    .status(200)
    .cookie('token', '', {
...
export const forgotPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  const { hashedPassword, resetPasswordExpireTime, resetToken } =
    generateResetPasswordToken();

  await database.query(
    'UPDATE users SET reset_password_token = $1, reset_password_expire = to_timestamp($2) WHERE email = $3',
    [hashedPassword, resetPasswordExpireTime / 1000, email]
  );
